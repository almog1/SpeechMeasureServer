...
class SpeechYoloVGGNetExtended(nn.Module):

    def __init__(self, classfication_model, c=7, b=2, k=10, dropout=0):
        super(SpeechYoloVGGNetExtended, self).__init__()
        self.c = c
        self.b = b
        self.k = k
        ...
        self.counter = 10
        self.last_layer = nn.Linear(last_input_size, self.c * (self.b * 3 + self.k) + self.counter)
        self.init_weight()
    ...
    def forward(self, x):
        ...
        last_layer_output = last_layer_output[:, :-self.counter]
        count_prediction = last_layer_output[:, -self.counter:]
        count_prediction = torch.sigmoid(count_prediction)
...
def create_speech_model(pretrained_model, arc, config_dict, dropout):
    if pretrained_model:
        if arc.startswith('VGG'):
            ...
            model_.load_state_dict(checkpoint['net'], strict=False)
...
def load_model1(dir, is_extended):
    ...
    if arc_type.startswith('VGG'):
        model_ = VGG(arc_type)
        if is_extended :
            speech_net = SpeechYoloVGGNetExtended(model_, config_dict["C"], config_dict["B"], config_dict["K"])
        else:
    ...
    return speech_net, checkpoint['epoch'], checkpoint['acc'], config_dict