...
def find_classes():
    keyword_list = ['AA', 'AE', 'AH', 'AO', 'AW', 'AY', 'EH', 'ER', 'EY', 'OW', 'OY', 'UH', 'UW', 'IH', 'IY']
    class_to_idx = {keyword_list[i]: i for i in range(len(keyword_list))}
    return keyword_list, class_to_idx
...
class SpeechYoloDataSet(Dataset):
    def __init__(self, this_root_dir, yolo_config, augment=False):
        """
        :param root_dir:
